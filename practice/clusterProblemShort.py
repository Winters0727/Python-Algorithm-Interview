...
def simulation(a, b): # 서버 감염 시뮬레이션 함수
    ...
    def calculate_percentile(server_checker, total): # 정상 서버의 비율을 구하는 함수
        return round(sum(server_checker.values())*100/total)
    ...
    def infesting(server_checker, infected_servers): # 서버 감염 및 클러스터 감염
    ...
    percentile = calculate_percentile(server_checker, total)
    day = 0
    ...
    while percentile > 40: # 정상적인 서버의 비율이 40%를 넘어선다면 = 감염된 서버의 비율이 60% 이하라면
        ...
        server_checker = infesting(server_checker, infected_servers) # 서버를 감염시키고
        percentile = calculate_percentile(server_checker, total) # 정상적인 서버의 비율을 구한다.
